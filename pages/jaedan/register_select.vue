...
const fetchData = async () => {
  try {
    ...
    // selectOrder.value = getOrder.data as WorkOrderModel | undefined;
    ...
    if (selectOrder.value) {
      ...
      useableWondan.value = useableWondanList.data as WondanModel[];
    } else {
...
const updateLotNo = () => {
  const selectedWondan = wondanCode.value.find(
    (wondan) => wondan.WONDAN_NAME === form.WONDAN_NAME,
  );
  if (selectedWondan) {
    form.LOT = selectedWondan.LOT_NO;
  }
};
...
const filteredWondan = computed(() => {
  ...
  return useableWondan.value.filter(
    (wondan) => wondan.WONDAN_NAME === selectOrder.value.WONDAN_NAME,
  );
});
